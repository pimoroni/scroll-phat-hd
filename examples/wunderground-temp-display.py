...
#
#
# Update to the wunderground-temp-display.py script to use the OpenWeather API and display on scrollphathd. This script shows the current temperature along with an indicator of
...
#
# Originally built by Mark Ehr, 1/12/18, and released to the public domain with no warranties expressed or implied. Or something along those lines. Feel free to use
#   this code any way that you'd like. If you want to give credit, that's great. 
#
...
try:
    ...
    import urllib2  #used to make web calls to OpenWeather
import json     #used to parse OpenWeather JSON data
...
# OpenWeather API key
OW_API_KEY = "YOUR_API_KEY_GOES_HERE"    #make sure to put your unique OpenWeather key in here
OW_API_KEY = os.environ.get("OW_API_KEY", OW_API_KEY) #or set the OW_API_KEY environment variable
...
OW_STATION = "5799841"                  #Kirkland, WA
...
BRIGHT = 0.2
DIM = 0.1
GUST_BRIGHTNESS = 0.3 #show gusts as a bright dot
...
if TEMP_SCALE == "F": #set max wind speed according to scale
    MAX_WIND_SPEED = 42.0 #MPH; default was set to 75.0, I lowered to 42 for better viewing of lower wind speeds
else: 
...
def get_weather_data():
    ...
    try:
      ...
      if TEMP_SCALE == "F": #Fahrenheit
          ...
          temperature = str(parsed_cond['main']['temp'])
