#!/usr/bin/env python
#
# binary_clock.py - A Python implementation of a binary clock for the Pimoroni Scroll Bot.
...
class BinaryClock(Clock):
  ...
  def _draw_binary(self, x, value):
    for y in range(self._hand_bits):
      self._SPHD.set_pixel(x, ((self._hand_bits - 1 - y) * 2), 1 if value & (1 << y) > 0 else self._brightness[x][y])
